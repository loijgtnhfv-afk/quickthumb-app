...
5. **Engine = `lib/nbp.ts`** → Replicate `google/nano-banana-2` (Gemini 3.1 Flash Image) since 2026-07-27; override with the `NBP_MODEL` env var to roll back without a deploy. Chosen over `google/nano-banana-pro` by a measured bake-off (`npm run bakeoff`, 15 real JP hooks/engine): JP text 15/15 vs 14/15, ~19s vs ~34s, $0.067 vs $0.134 per image. **The user PICKS which styles to generate** (multi-select, default 4 of 12 — `body.concept_keys`, resolved by `selectConcepts()`; omitting the field falls back to DEFAULT_CONCEPT_KEYS — the original four — NOT all ten, because under per-image credits an omitted field would silently bill a legacy caller 10 instead of 4). For each picked concept it builds a prompt that RESERVES a text zone (so the hero never occludes the hook), appends the weekly clause from `lib/gallery-insights.ts`, passes the face photo as `image_input` for identity preservation, generates 16:9 @2K, and pins the result to 1280×720 via sharp. Concepts run in parallel via `Promise.allSettled` (one concept failing — e.g. an NBP safety refusal — doesn't sink the batch), and every image that fails is refunded individually.
   - **The 10 styles** live in `lib/concept-keys.ts` (shared, dependency-free, so the client picker and the server prompts cannot drift) with prompts in `lib/nbp.ts`. Originals: face-surprise / jp-telop / global-clean[EN] / action. Added 2026-07-27: object-spotlight / calm-authority / split-compare / risk-warning / soft-lifestyle / night-cinematic[EN], then anime-style / game-live (Vtuber + gaming — the 3rd-largest genre on the SAMUNE corpus was Vtuber at 443/2667, and every other concept is photographic, so an illustrated-avatar creator had nothing to pick). anime-style is the ONE concept that redraws the person on purpose; it uses animeHeroClause(), kept separate so no photographic concept can inherit "draw this person". **APPEND ONLY** — files are named `thumb-{index+1}.png` from a concept's position, so inserting or reordering renames existing slots.
...
**Cost ≈ $0.067/image** (nano-banana-2 @2K); a default 4-style generation ≈ $0.27, all 12 ≈ $0.80. Iterate prompts/concepts offline with `scripts/preview-nbp.ts` (needs only `REPLICATE_API_TOKEN`; output in `.preview-nbp/`).
...
Tables:
- `profiles` — id (FK auth.users), plan ('free' default), **`image_credits_used` / `image_credits_limit`** (int). **The quota unit is ONE GENERATED IMAGE, not one generation** (changed 2026-07-27, when the style picker grew to 10 and a single request could produce 10 images). Free default = 4 images (the old "1 generation × 4 images", unchanged in value). Pro = `STRIPE_PRO_IMAGE_CREDITS`, default 80. The old `generations_used` / `generations_limit` columns still exist but are DEAD — nothing reads them; drop them once the new columns have been stable for a while. Stripe billing columns (`stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, `current_period_end`) are added by the PHASE2.md §2 migration when billing is activated.
...
- Stripe (OPTIONAL — billing is inert until all are set; see PHASE2.md §2): `STRIPE_SECRET_KEY`, `NEXT_PUBLIC_STRIPE_PRICE_ID`, `STRIPE_WEBHOOK_SECRET`, optional `STRIPE_PRO_IMAGE_CREDITS` (default 80) and `FREE_IMAGE_CREDITS` (default 4). ⚠️ The old `STRIPE_PRO_GENERATIONS_LIMIT` is NO LONGER READ — the name changed with the unit (2026-07-27). Reusing it would have handed Pro subscribers a stale `20` meaning 20 images instead of 80.
...
- ⚠️ **SUPERSEDED 2026-07-27 — the descriptor loop is RETIRED.** Its consumer (`descriptorClauseFor()`) died with the Flux/Satori pipeline on 2026-06-03, but the weekly cron kept running and kept costing money for ~8 weeks while changing nothing. `refresh-descriptors.yml` is now `workflow_dispatch`-only. The live learning loop is **`scripts/learn-from-gallery.ts` + `.github/workflows/learn-from-gallery.yml`** (Mondays): it samples curated Japanese thumbnails from thumbnail-gallery.net via its WordPress REST API, has Claude Vision describe the visual tendencies, and writes one short colour/light/mood clause per style to `references/gallery-insights.json`. That file IS read on every generation (`lib/gallery-insights.ts` → `/api/generate`), which is the whole point — a learning loop not wired into generation is just a bill. Images are never stored; clauses touching layout or typography are rejected by a filter so they can't fight a validated prompt.
- **Reference style descriptors** (historical, 2026-05-31 → 2026-07-27). Collected via per-style YouTube **search** — NOT the trending chart, which was too noisy: trending + `categoryId` fed music/movie/Shorts into the vlog/tech buckets and Vision refused. Now `STYLE_QUERIES` per style, `order=viewCount`, `videoDuration=medium` (excludes vertical Shorts whose pillarboxed thumbs taught a wrong vertical/triptych look), gaming pinned to `videoCategoryId=20` (else "let's play"/viewCount pulls kids-edutainment like Sesame Street). Weekly GHA cron `refresh-descriptors.yml` (Mondays): collect → Claude Vision extract → auto-commit `descriptors.json` → Vercel redeploys. `descriptorClauseFor()` in `app/api/generate/route.ts` appends each style's descriptor to its Flux prompt; `extract-descriptors` drops Vision refusals via `isCleanDescriptor`. GH Actions secrets `YOUTUBE_API_KEY` + `ANTHROPIC_API_KEY` are set. Tune queries in `scripts/collect-trending-thumbs.ts`; details in `references/README.md`.
...
4. **Activate Stripe** (code is shipped + dormant) — PHASE2.md §2 checklist; test in Stripe TEST mode first. **Legal prerequisite (founder, HIGH):** before charging, terms/privacy need a legal/trading entity name, governing law + jurisdiction, refund/cancellation policy, and a Japanese 特定商取引法に基づく表記 page — a TEMPLATE now exists in the repo (`public/tokushoho.html` + new Operator / Paid-plans-refunds / Governing-law sections in `terms.html` & `privacy.html`), with the business details left as highlighted `[TO FILL IN]` placeholders. The founder must fill: entity name, address, phone, responsible person, governing law + court, refund policy, and final prices — then delete the template banner in `tokushoho.html`. The footer link to `/tokushoho.html` is gated on `NEXT_PUBLIC_STRIPE_PRICE_ID` (appears once billing is on).
